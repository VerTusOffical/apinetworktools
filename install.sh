...
# Скрипт для быстрой установки мульти-инструмента
...
echo -e "${BLUE}==================================${NC}"
echo -e "${BLUE}  Установка Linux Мульти-Инструмента${NC}"
echo -e "${BLUE}==================================${NC}"
...
echo -e "${BLUE}Загрузка скрипта...${NC}"

# Загрузка setup.sh
curl -sSL https://raw.githubusercontent.com/USERNAME/REPO/main/setup.sh -o setup.sh 2>/dev/null

if [ $? -ne 0 ]; then
    # Если загрузка неуспешна, создаем локальную копию скрипта
    echo -e "${RED}Не удалось загрузить скрипт. Создание локальной копии...${NC}"
...
main_menu() {
    ...
    echo -e "\033[1;35m       LINUX МУЛЬТИ-ИНСТРУМЕНТ        \033[0m"
    echo -e "\033[1;35m======================================\033[0m"
...
echo -e "${BLUE}Запуск Мульти-Инструмента...${NC}"
